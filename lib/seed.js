...
const { initTables, createProduct, getAdminByUsername, createAdminUser, getAllProducts, getDb } = require('./db');
...
async function seed() {
  ...
  if (!adminPassword) {
    ...
    console.error('Set it in .env.local or pass it when running: ADMIN_PASSWORD=your_password node lib/seed.js');
    process.exit(1);
  }
  ...
  if (!existingAdmin) {
    ...
    await createAdminUser(adminUsername, hash);
  ...
  if (products.length === 0) {
    ...
    console.log(`✅ Created ${newProducts.length} sample products`);
    ...
    for (const sampleOrder of sampleOrders) {
      ...
      const daysOffset = sampleOrder.daysAgo;
      ...
      await pool.query(
        `INSERT INTO orders (id, customer_name, customer_email, total, status, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [orderId, sampleOrder.customer_name, sampleOrder.customer_email, Math.round(total * 100) / 100, sampleOrder.status, createdAt]
      );
      ...
      for (const item of items) {
        await pool.query(
          `INSERT INTO order_items (id, order_id, product_id, product_name, quantity, price_at_time)
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [uuidv4(), orderId, item.product_id, item.product_name, item.quantity, item.price_at_time]
        );
      }
    }
